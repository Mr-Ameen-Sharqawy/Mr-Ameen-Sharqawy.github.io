...
import { firebaseAuth, onAuthStateChanged, signInWithUsername, signOutStudent, usernameFromFirebaseEmail } from "./lib/firebase-auth";
import "./style.css";
...
function GradeHome({ onSelect, studentUsername, onSignOut }: { onSelect: (grade: Grade) => void; studentUsername: string; onSignOut: () => void }) {
  return <main className="grade-home" dir="rtl"><header className="public-header"><div className="brand"><span>VOCABULARY JOURNEY</span><strong>اختيار الصف</strong></div><div className="public-user-actions"><span className="public-chip">{studentUsername}</span><button className="public-logout" onClick={onSignOut}>خروج</button></div></header><section className="intro"><div className="intro-icon"><BookOpen size={29} /></div><p>اختر رحلتك التعليمية</p><h1>أي صف ستتعلم اليوم؟</h1><span>يُحفظ تقدمك المحلي منفصلًا لكل صف ووحدة.</span></section><section className="grade-grid">{(Object.keys(gradeMeta) as Grade[]).map((grade) => <article className={`grade-card ${gradeMeta[grade].accent}`} key={grade}><div className="grade-card-icon">{grade === "grade5" ? <BookOpen size={26} /> : <Sparkles size={26} />}</div><p>{grade === "grade4" ? "PRIMARY 4" : grade === "grade5" ? "PRIMARY 5" : "PRIMARY 6"}</p><h2>{gradeMeta[grade].english}</h2><span>{gradeMeta[grade].description}</span><small>{gradeMeta[grade].note}</small><button onClick={() => onSelect(grade)}>ابدأ {gradeMeta[grade].english}<ChevronLeft size={17} /></button></article>)}</section><p className="static-note">بيانات الدخول محمية عبر Firebase، ويُحفظ تقدمك محليًا على جهازك.</p></main>;
}
...
function App() { const [email, setEmail] = useState<string | null | undefined>(undefined); const [grade, setGrade] = useState<Grade | null>(() => gradeFromHash()); useEffect(() => onAuthStateChanged(firebaseAuth, (user) => setEmail(user?.email ?? null)), []); useEffect(() => { const update = () => setGrade(gradeFromHash()); window.addEventListener("hashchange", update); return () => window.removeEventListener("hashchange", update); }, []); if (email === undefined) return <main className="auth-page"><p className="auth-loading">جارٍ تجهيز بوابة الطلاب...</p></main>; if (!email) return <StudentLogin />; const studentUsername = usernameFromFirebaseEmail(email); const leave = () => { window.history.pushState(null, "", window.location.pathname + window.location.search); setGrade(null); }; const logout = () => { void signOutStudent(); }; return grade ? <StudentCourse grade={grade} onBack={leave} studentUsername={studentUsername} onSignOut={logout} /> : <GradeHome onSelect={(next) => { window.location.hash = next; }} studentUsername={studentUsername} onSignOut={logout} />; }
